...
import ni.devotion.multidataparser.image.ImageLoader.Companion.screenHeight
import ni.devotion.multidataparser.image.ImageLoader.Companion.screenWidth
...
object ImageManager {
    ...
    fun downloadBitmapFromURL(imageUrl: String): Bitmap? {
        val inputStream = BufferedInputStream(URL(imageUrl).openConnection().getInputStream())
    ...
    private fun calculateInSampleSize(options: BitmapFactory.Options, reqWidth: Int, reqHeight: Int): Int {
        ...
        var placeHolderSize = 1
        if (height > reqHeight || width > reqWidth) {
            val halfHeight: Int = height / 2
            val halfWidth: Int = width / 2
            while (halfHeight / placeHolderSize >= reqHeight && halfWidth / placeHolderSize >= reqWidth) placeHolderSize *= 2
        }
